...
namespace Lazy.Abp.CoinKit.CoinPurchaseds
{
    public class CoinPurchasedManager : DomainService, IUnitOfWorkEnabled
    {
        ...
        public CoinPurchasedManager(
            ICoinPurchasedRepository repository,
        ...
        /// </summary>
        [UnitOfWork]
        public async Task<CoinPurchased> RechargeAsync(
            Guid? tenantId,
            Guid userId,
            Guid coinProductId,
            int productQuantity,
        ...
        )
        {
            using (CurrentTenant.Change(tenantId))
            {
                var coinPurchased = new CoinPurchased(GuidGenerator.Create(), tenantId, userId, coinProductId, productQuantity, orderId, coinTotal, paidAmount);
